		...
		for (let animal of ShelterAnimal) {
			console.log(animal);
			  const adoptionStatus = animal.ifAdopted === 1 ? '已領養' : '未領養';
			    const id = animal.userAccount === null ? '尚未有領養人' : animal.userAccount;
			tbody.innerHTML += `
				<tr>
				<td id="animalName${animal.animalName}">${animal.animalName}</td>
				<td id="animalNum${animal.animalNameber}">${animal.animalNumber}</td>
				<td id="userId${animal.userId}">${id}</td>
				<td><a href="applylist.html?id=${animal.animalId}" onclick="apply(${animal.animalId})">查看申請人</a></td>
				<td class="button-td">
					<button type="button" class="btn-edit btn btn-success" onclick="editItem(${animal.animalId})"><i class="fa-regular fa-pen-to-square fa-fade"></i></button>
				</td>
				<td class="button-td">
					<button type="button" class="btn-delete btn btn-danger" onclick="deleteClick(${animal.animalId})"><i class="fa-regular fa-trash-can fa-bounce"></i></button>
				</td>
